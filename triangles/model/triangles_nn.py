...
class Core(nn.Module):
    ...
    def out_features(self) -> int:
        return self.embedding_size*(self.max_triangles + 1)
    ...
    def __call__(self, observations: DictArrayType) -> Array:
        ...
        embed = nn.Embed(num_embeddings=2, features=self.embedding_size)

        batch_size = error_inputs.shape[0]
        error_inputs = jnp.expand_dims(CNN()(error_inputs), 1)

        error_token = Encoder(embed_size=self.embedding_size, embed_idx=0, embed=embed)(error_inputs)
