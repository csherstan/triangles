...
class TriangleEnv(Env[ActType, ObsType]):
        ...
        self.action_space = spaces.Dict(
            {
                "data": spaces.Dict(
                    {
                        "index": spaces.Box(low=0, high=np.inf, dtype=np.int32),
                        "triangle": triangle_space,
                    }
                ),
                "op": spaces.Discrete(3),
            }
        )
    ...
    def step(
            ...
            case self.Op.ADD:
                if self.energy >= self.add_cost:
                    self.triangles.append(
                        Triangle.from_array(action["data"]["triangle"])
                    )
        ...
        rendered = render_triangles(
            self.triangles, height=self.height, width=self.width
        )
    ...
    def reset(
        ...
        *,
        ...
        self.triangles = (
            [Triangle.from_array(t) for t in options["triangles"]]
...
def render_triangles(triangles: List[Triangle], width: int, height: int) -> np.ndarray:
    ...
    for triangle in triangles:
        draw.polygon(
            [
                (scale_int(v.x, width), scale_int(v.y, height))
                for v in triangle.vertices
            ],
            (
                scale_rgba(triangle.r),
                scale_rgba(triangle.g),
                scale_rgba(triangle.b),
                scale_rgba(triangle.a),
            ),
        )
