...
def rw(idx: int, shutdown: EventClass, env_factory: EnvFactoryType, queue, config: RWConfig):
  with jax.default_device(jax.devices('cpu')[0]):
    assert jnp.array([0]).devices().pop().platform == "cpu"
    rw_(idx, shutdown, env_factory, queue, config)


def rw_(rw_id: int, shutdown: EventClass, env_factory: EnvFactoryType, queue: Queue, config: RWConfig):
  """
...
def eval_process(shutdown: EventClass, env_factory: EnvFactoryType, model_queue: multiprocessing.Queue,
    ...
    while not shutdown.is_set():
      try:
        ...
        if model_clock % config.eval_frequency == 0:
          ...
          eval_step(env_factory, params, model_clock, config, next(rng_gen), metric_writer)

      except Empty:
        pass


def eval_step(env_factory: EnvFactoryType, policy_params: VariableDict, model_clock: int, config: ExpConfig,
              rng_key: Array,
...
def create_policy_state(env: gym.Env, config: ExpConfig, rng_key: Array) -> TrainState:
  rng_gen = rng_seq(rng_key=rng_key)
...
def create_init_state(config: ExpConfig, env: gym.Env, rng_key: Array) -> SACModelState:
  rng_gen = rng_seq(rng_key=rng_key)
  policy_state = create_policy_state(env, config, next(rng_gen))
...
def train(config: ExpConfig, env_factory: EnvFactoryType) -> None:
  """
  ...
  output_dir = Path(f"data/classic_control/{env.spec.id}/{datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f')}")
...
def watch(config: ExpConfig, checkpoint: Path, env_factory: EnvFactoryType) -> None:
  with jax.default_device(jax.devices('cpu')[0]):
    rng_gen = rng_seq(seed=time.time_ns())
...
def main(config: ExpConfig, args, env_factory: EnvFactoryType) -> None:
  match args.mode:
    case "train":
      train(config, env_factory)

    case "watch":
      assert args.checkpoint
      watch(config, args.checkpoint, env_factory)
