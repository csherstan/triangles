...
def space_to_reverb_spec(space: spaces.Space):
  if isinstance(space, spaces.Box):
    ...
    return tf.TensorSpec(shape=(1, ), dtype=tf.float32)
...
def main(name: str, config: ExpConfig, args, env_factory: EnvFactoryType, policy_factory: PolicyFactoryType,
  ...
  match args.mode:
    case "train":
      train(name=name, config=config, env_factory=env_factory, policy_factory=policy_factory, sac_state_factory=sac_state_factory)
...
def slice_leaves(data, slicer: slice) -> Any:
  return jax.tree_map(lambda entry: entry[slicer][0], data)