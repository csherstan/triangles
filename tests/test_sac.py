...
@pytest.mark.parametrize(
    ["alpha_params", "logits", "expected"],
    [
        [
            {"alpha": jnp.log(jnp.array([0.5]))},
        ...
        [
            {
                "alpha": {
                    "0.5": jnp.log(jnp.array([0.5])),
                    "0.1": jnp.log(jnp.array([0.1])),
                }
            },
            {
                "0.5": jnp.array([[1.0], [2.0], [3.0]]),
                "0.1": jnp.array([[1.0], [2.0], [3.0]]),
            },
...
def test_compute_entropy_bonus(
    alpha_params: AlphaType, logits: MappingArrayType, expected: MappingArrayType
):
