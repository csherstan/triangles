...
@pytest.mark.parametrize(
    ["env_name", "state_factory"],
    [
        ("Pendulum-v1", continuous.policy_factory, continuous.sac_state_factory),
        (
            "MixedAction2D-v0",
            mixed_action.policy_factory,
            mixed_action.sac_state_factory,
        ),
    ],
)
def test_that_it_runs(
    env_name: str, policy_factory: PolicyFactory, state_factory: SACStateFactory
):
    env = gym.make(env_name)
    policy = policy_factory(env)
    config = ExpConfig()
    sac_state = state_factory(
        config=config, env=env, policy=policy, rng_key=jnp.random.PRNGKey(0)
    )
